...
def solve_sudoku(board):
    ...
    cols = convert_to_sets(cols, rows) # Convert cols into dictionary of sets for easy access from cols to rows
...
def solve(cols, rows, solution_set=[]): # Dancing Links X main solver algorithm
    if not cols: # Return solution set when no columns left (base case)
        ...
        for selected_row in list(cols[min_col]): # Iterate thorugh rows, selecting a row to cover every time
            ...
            cols, cells_to_restore = cover(cols, rows, selected_row) # Cover selected row and intersecting cols
            for sol in solve(cols, rows, solution_set): # Recursively solve the reduced matrix
                yield sol
            cols = uncover(cols, cells_to_restore) # Uncover
            solution_set.pop() # Remove from solution set

def cover(cols, rows, selected_row): # Cover function
...
def uncover(cols, cells_to_restore): # Uncover function
    for col, row in cells_to_restore: # Loop through cells to restore
