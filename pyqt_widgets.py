...
from PyQt6.QtWidgets import QMainWindow, QLabel, QPushButton, QToolBar, QMenu, QComboBox, QProgressBar, QWidget, \
    QTextEdit, QLineEdit, QTableWidget, QAbstractScrollArea, QTableWidgetItem, QAbstractItemView, QHeaderView
...
class Button(QPushButton):  # Screen Button
    ...
    def maximise(self, factor):
        self.setGeometry(int(self._orig_x * factor), int(self._orig_y * factor), int(self._orig_width * factor),
                         int(self._orig_height * factor))
...
class BackButton(CircularButton):  # Back button to return to previous page
    def __init__(self, window, command):
        super().__init__(window, 925, 15, 60, 60, QIcon("resources/back.svg"),
                         command)  # Back button is just a circular button with an arrow image


class TextEdit(
    QTextEdit):  # TextEdit object to display text, e.g. help menu and displaying contents of appearance preset / game files
    def __init__(self, window, x, y, width, height, background_colour, border_width, font_family, font_size):
        ...
        self.setGeometry(x, y, width, height)  # Set position and dimensions
        self.setStyleSheet(
            f"background: {background_colour}; border: {border_width}px solid black;")  # Set background colour and border width
        self.setFont(QFont(font_family, font_size))  # Set font
...
class Rect(QWidget):  # Rectangle with background colour widget
    ...
    def maximise(self, factor):
        self.setGeometry(int(self._orig_x * factor), int(self._orig_y * factor), int(self._orig_width * factor),
                         int(self._orig_height * factor))
        self.setStyleSheet(
            f"background: {self._background_colour}; border: {int(self._orig_border_width * factor)}px solid black;")
...
class ToolBar(QToolBar):  # Toolbar widget
    ...
    def maximise(self, factor):
        self.setIconSize(
            icon_size := QSize(int(self._orig_icon_size.width() * factor), int(self._orig_icon_size.height() * factor)))
...
class TableWidget(QTableWidget):  # Table widget, used to display rankings in leaderboard
    def __init__(self, window, x, y, width, height, font_family, font_size, background_colour, num_rows, num_cols):
        ...
        self.setEditTriggers(
            QAbstractItemView.EditTrigger.NoEditTriggers)  # Do not allow the user to edit the leaderboard table
        self.horizontalHeader().setSectionResizeMode(
            QHeaderView.ResizeMode.ResizeToContents)  # Resize horizontal header columns to contents
        self.verticalHeader().setSectionResizeMode(
            QHeaderView.ResizeMode.ResizeToContents)  # Resize vertical header rows to contents
        self.setSizeAdjustPolicy(
            QAbstractScrollArea.SizeAdjustPolicy.AdjustToContents)  # Resize cell size adjust to contents

    def load_data(self, headings,
                  data):  # Method to load table with data when given headings (list) and data (list[tuple])
        ...
        for row in range(self.rowCount()):  # Loop through rows
            for col in range(self.columnCount()):  # Loop through cols
                ...
                value = data[row][col] if 0 <= row < len(data) and 0 <= col < len(
                    data[0]) else None  # Get value if (row, col) in range
        ...
        self.setSortingEnabled(
            True)  # Enable sorting so user can click column headers to sort in ascending / descending order
        self.sortByColumn(1,
                          Qt.SortOrder.DescendingOrder)  # Sort ratings column (column 1) in descending order by default
...
class Screen(QMainWindow):  # Screen widget, all GUI screens inherit from Screen
    ...
    def __init__(self, application, max_size: QSize, title_name, create_button):
        ...
        self.setStyleSheet(
            f"background: {self._application.account.app_config.background_colour};")  # set background colour
        self.statusBar().setFont(QFont(self._application.account.app_config.regular_font,
                                       self._status_bar_orig_font_size))  # Set font of status bar (to show errors)
    ...
    def resizeEvent(self, event):
        ...
        if not self.isMaximized():  # If screen is not maximised
            self.statusBar().setFont(QFont(self._application.account.app_config.regular_font,
                                           int(self._status_bar_orig_font_size * self._resize_factor)))
            ...
            self.statusBar().setFont(
                QFont(self._application.account.app_config.regular_font, self._status_bar_orig_font_size))
            for widget in self._widgets:  # Loop through all widgets
