...
from rating_calc import get_title, RECOMMENDED_RATINGS, \
    average_time_to_complete  # import functions from rating calculation file
...
class Application:  # Application class, used to manage accounts and all account-related features
    ...
    def sign_in(self, options):  # Method to sign in
        ...
        if database.password_at(username)[0][0] == database.encrypt_password(
                password):  # Check if password entered is correct
    ...
    def __curr_milestone_rank(self,
                              milestone):  # Method to get current milestone rank (1 to 7) of any milestone collection
    ...
    def update_milestone(self, data):  # Method to update milestone after each game
        ...
        if won:  # Check if user has won the game
            ...
            milestone = database.milestone(self.__account.username,
                                           bs := f"milestone_{board_size}x{board_size}")  # get current milestone
            new_milestone = milestone + GameMilestones.MILESTONE_GAIN[difficulty] * (
                2 if mode == "Killer" else 1)  # Calculate new milestone
            ...
            if new_rank != old_rank:  # Check if milestone rank changed
                ...
                # Update the index to signify that a milestone reward needs to be claimed
                database.set_milestone_claimed(self.__account.username,
                                               claimed[:idx] + "1" + claimed[idx + 1:])
    ...
    def claim_reward(self, data):  # Method to claim reward for a milestone (from game milestone screen)
        ...
        database.set_milestone_claimed(self.__account.username,
                                       claimed[:idx] + "0" + claimed[idx + 1:])  # Update the index to claim reward
        ...
        if reward is not None and reward[0] == "H":
            ...
            database.set_bonus_hints(self.__account.username,
                                     bonus_hints + reward[1])  # Add the bonus hints earned from the reward
