...
class HomeScreen(Screen):
    ...
    def __init__(self, application, max_size):  # Constructor
        ...
        super().__init__(application=application, max_size=max_size, title_name=None,
                         create_button=False)  # Inheritance
        ...
        self.__create_new_game_button = Button(self, "CREATE NEW GAME", 300, 250, 400, 50,
                                               self._application.account.app_config.regular_font, 25,
                                               self.__create_new_game)
        self.__create_new_game_button.setStyleSheet(
            f"background: {self._application.account.app_config.colour2}; border: 2px solid black;")

        # Open existing game button
        self.__open_existing_game_button = Button(self, "OPEN EXISTING GAME", 300, 320, 400, 50,
                                                  self._application.account.app_config.regular_font, 25,
                                                  self.__open_existing_game)
        self.__open_existing_game_button.setStyleSheet(
            f"background: {self._application.account.app_config.colour2}; border: 2px solid black;")

        # Leaderboard button
        self.__leaderboard_button = Button(self, "LEADERBOARD", 300, 390, 400, 50,
                                           self._application.account.app_config.regular_font, 25,
                                           self.__leaderboard_screen)
        self.__leaderboard_button.setStyleSheet(
            f"background: {self._application.account.app_config.colour2}; border: 2px solid black;")

        # Toolbar on right hand side
        self.__toolbar = ToolBar(self, size := QSize(60, 60), self._application.account.app_config.colour3,
                                 font_family := self._application.account.app_config.regular_font, font_size := 15)
        self.addToolBar(Qt.ToolBarArea.RightToolBarArea, self.__toolbar)
        self.__toolbar.addAction(
            Action(self, QIcon("resources/exit.svg"), "Quit", self.__quit_game, False))  # Add quit button
        ...
        self.__toolbar.addWidget(MenuButton(self, QIcon("resources/account.svg"), size, QFont(font_family, font_size),
                                            options))  # Add account button
        self.__toolbar.addWidget(MenuButton(self, QIcon("resources/settings.svg"), size, QFont(font_family, font_size),
                                            [("Customise GUI", self.__customise_gui),
                                             ("Manage Account", self.__manage_account)]))  # Add settings button
        self.__toolbar.addWidget(MenuButton(self, QIcon("resources/stats.svg"), size, QFont(font_family, font_size),
                                            [("Show Stats", self.__view_stats),
                                             ("Game Milestones", self.__game_milestones)]))  # Add stats button
        self.__toolbar.addAction(
            Action(self, QIcon("resources/help.svg"), "Help", self.__help_screen, False))  # Add help button

        # Current account
        self.__account_label = Label(self,
                                     "Not Signed In" if not self._application.signed_in else f"Signed in as {self._application.account.username} ({self._application.account.title} | {self._application.account.rating})",
                                     0, 0, 450, 50, self._application.account.app_config.regular_font, 15)

        # Add to widgets (for maximising)
        self._widgets += [self.__title, self.__create_new_game_button, self.__open_existing_game_button,
                          self.__leaderboard_button, self.__toolbar, self.__account_label]
...
class ConfigGameScreen(Screen):  # Create new game screen
    ...
    def __init__(self, application, max_size):  # Constructor
        ...
        super().__init__(application=application, max_size=max_size, title_name="CREATE NEW GAME",
                         create_button=True)  # Inheritance

        # Create labels for board settings
        for idx, label in enumerate(("MODE: ", "BOARD SIZE: ", "DIFFICULTY: ", "TIMED: ", "HARDCORE: ")):
            label_obj = Label(self, label, 50, 150 + 75 * idx, 300, 100,
                              self._application.account.app_config.regular_font, 24)
            self._widgets.append(label_obj)
        self.__hardcore_label = Label(self,
                                      "(Hardcore enabled = No auto notes or hints allowed\n *Required to be on the leaderboard)",
                                      50, 150 + 75 * 4 + 60, 500, 75, self._application.account.app_config.regular_font,
                                      14)

        # Create menu for mode
        self.__mode_menu = ComboBox(self, 330, 175, 200, 50, self._application.account.app_config.regular_font, 20,
                                    ["Normal", "Killer"])
        self.__mode_menu.setStyleSheet(
            f"background: {self._application.account.app_config.colour2}; border: 2px solid black;")
        self.__mode_menu.activated.connect(self.__update_gamemode_infobox)

        # Create menu for board size
        self.__board_size_menu = ComboBox(self, 330, 250, 200, 50, self._application.account.app_config.regular_font,
                                          20, ["4x4", "6x6", "9x9", "12x12", "16x16"])
        self.__board_size_menu.setStyleSheet(
            f"background: {self._application.account.app_config.colour2}; border: 2px solid black;")
        self.__board_size_menu.activated.connect(self.__update_gamemode_infobox)

        # Create menu for difficulty
        self.__difficulty_menu = ComboBox(self, 330, 325, 200, 50, self._application.account.app_config.regular_font,
                                          20, ["Easy", "Medium", "Hard", "Expert"])
        self.__difficulty_menu.setStyleSheet(
            f"background: {self._application.account.app_config.colour2}; border: 2px solid black;")
        self.__difficulty_menu.activated.connect(self.__update_gamemode_infobox)

        # Create menu for timed
        self.__timed_menu = ComboBox(self, 330, 400, 200, 50, self._application.account.app_config.regular_font, 20,
                                     ["Yes", "No"])
        self.__timed_menu.setStyleSheet(
            f"background: {self._application.account.app_config.colour2}; border: 2px solid black;")

        # Create menu for hardcore
        self.__hardcore_menu = ComboBox(self, 330, 475, 200, 50, self._application.account.app_config.regular_font, 20,
                                        ["Yes", "No"])
        self.__hardcore_menu.setStyleSheet(
            f"background: {self._application.account.app_config.colour2}; border: 2px solid black;")

        # Create text box to show info about the gamemode that the user is selecting
        self.__gamemode_info = TextEdit(self, 650, 175, 300, 275, self._application.account.app_config.colour2, 3,
                                        self._application.account.app_config.regular_font, 18)

        # Play button
        self.__play = Button(self, "PLAY GAME", 650, 475, 300, 50, self._application.account.app_config.regular_font,
                             20, self.__play_game)
        self.__play.setStyleSheet(
            f"background: {self._application.account.app_config.colour2}; border: 2px solid black;")

        # Add to widgets (for maximising)
        self._widgets += [self.__play, self.__mode_menu, self.__difficulty_menu,
                          self.__timed_menu, self.__board_size_menu, self.__hardcore_menu, self.__hardcore_label,
                          self.__gamemode_info]
    ...
    def __play_game(self):  # Play game
        try:
            if (difficulty := self.__difficulty_menu.currentText()) and (timed := self.__timed_menu.currentText()) and \
                    (board_size := self.__board_size_menu.currentText()) and (
            mode := self.__mode_menu.currentText()) and \
                ...
                if (mode, board_size, difficulty) in Game.DISABLED_GAMEMODES:  # Check if gamemode is disabled
                    raise ApplicationError(
                        f"*{mode} {board_size} {difficulty} is disabled due to generation issues")  # Raise error
                else:
                    self.setWindowTitle(
                        "Board Generation in Progress")  # Tell user that the board is currently being generated
                    self.play_game_signal.emit(
                        [mode, difficulty, int(board_size.split("x")[0]), True if timed == "Yes" else False,
                         True if hardcore == "Yes" else False])
            else:
    ...
    def __update_gamemode_infobox(
            self):  # Method to update gamemode info box every time user clicks the first three comboboxes
        if (difficulty := self.__difficulty_menu.currentText()) and (
        board_size := self.__board_size_menu.currentText()) and \
            ...
            if (mode, board_size, difficulty) in Game.DISABLED_GAMEMODES:  # Check if gamemode has been disabled
                ...
                time_taken = self._application.get_average_time_to_complete(mode, board_size,
                                                                            difficulty)  # Get average time to complete
...
class OpenGameScreen(Screen):  # Open game screen
    ...
    def __init__(self, application, max_size):  # Constructor
        ...
        super().__init__(application=application, max_size=max_size, title_name="OPEN EXISTING GAME",
                         create_button=True)  # Inheritance

        # Play button
        self.__play = Button(self, "PLAY GAME", 675, 290, 200, 50, self._application.account.app_config.regular_font,
                             20, self.__play_game)
        self.__play.setStyleSheet(
            f"background: {self._application.account.app_config.colour2}; border: 2px solid black;")

        # Choose game label
        self.__choose_game = Label(self, "CHOOSE A GAME: ", 50, 150, 300, 100,
                                   self._application.account.app_config.regular_font, 20)

        # Choose game menu (multiple choice drop down box)
        self.__choose_game_menu = ComboBox(self, 50, 230, 400, 50, self._application.account.app_config.regular_font,
                                           15, self._application.get_game_files())
        self.__choose_game_menu.setStyleSheet(
            f"background: {self._application.account.app_config.colour2}; border: 2px solid black;")
        self.__choose_game_menu.activated.connect(self.__show_game_info)

        # Game info box to display game settings
        self.__game_info = TextEdit(self, 50, 300, 400, 235, self._application.account.app_config.colour2, 2,
                                    self._application.account.app_config.regular_font, 18)
    ...
    def __show_game_info(self):  # Show game info
        if file := self.__choose_game_menu.currentText():  # Check if user chose a file to open
            ...
            self.__game_info.setText("\n".join(
                [f"{label}: {stats[label.lower()]}" for label in labels]))  # Render label-stat pairs in TextEdit
        else:
...
class GameScreen(Screen):  # Main game screen
    ...
    NUM_FONT_SIZES = {4: 20 * 9 // 4, 6: 20 * 9 // 6, 9: 20, 12: 20 * 9 // 12,
                      16: 20 * 9 // 16}  # Font sizes for number
    ...
    def __init__(self, application, max_size, game: Game, time_to_generate):  # Constructor
        ...
        self.__info_button = CircularButton(self, 845, 15, 60, 60, QIcon("resources/info.svg"),
                                            self.__toggle_info_screen)

        # Undo button
        self.__undo_button = CircularButton(self, 610, 470, 58, 58, QIcon("resources/undo.svg"), self.__undo_move)

        # Auto note button
        self.__auto_note_button = CircularButton(self, 677, 470, 58, 58, QIcon("resources/auto_note.svg"),
                                                 self.__show_auto_note)

        # Label to show number of auto notes left
        self.__num_auto_notes_label = Label(self, "", 681, 535, 58, 58,
                                            self._application.account.app_config.regular_font, 15)
        ...
        if not self._application.signed_in:
            ...
            bonus_hint_str = f'(+{bonus_hints})' if (
                                                        bonus_hints := self._application.account.bonus_hints) != 0 and self.__game.num_hints_left > 0 else ''
        self.__num_hints_label.setText(f"{self.__game.num_hints_left} {bonus_hint_str}")

        # Notes button 
        self.__notes_button = CircularButton(self, 811, 470, 58, 58, QIcon("resources/notes_off.svg"),
                                             self.__toggle_notes_mode)  # notes
        self.__notes_button.setIconSize(QSize(53, 53))
        self.__notes_button.setStyleSheet("border-radius: 29px; border: 5px solid black;")

        # Resign button
        self.__resign_button = CircularButton(self, 878, 470, 58, 58, QIcon("resources/resign.svg"),
                                              partial(self.__show_end_screen, False))  # resign
        ...
        # Create board cover
        self.__board_cover = Rect(self, self.STARTX + self.PADDING - 3, self.PADDING - 3,
                                  width := self.GRIDSIZE * self.__game.board_size + 12,
                                  height := self.GRIDSIZE * self.__game.board_size + 12,
                                  self._application.account.app_config.colour2, 5)

        # Create paused label
        self.__paused_label = Label(self.__board_cover, "GAME PAUSED", 0, 0, width, height,
                                    self._application.account.app_config.regular_font, 24)
        ...
        self._widgets += [self.__timer, self.__info_label, self.__info_button, self.__undo_button,
                          self.__auto_note_button, self.__hint_button, self.__num_auto_notes_label,
                          self.__num_hints_label, self.__notes_button,
    ...
    def __show_border(self, board_size, matrix_size):  # Method to show board border
        ...
        big_border = Border(self, self.STARTX + self.PADDING - 3, self.PADDING - 3,
                            self.GRIDSIZE * board_size + 6 + 6, self.GRIDSIZE * board_size + 6 + 6,
                            3)  # Border around the board
        ...
        for row in range(matrix_size[1]):  # Loop through all matrices on the board
            for col in range(matrix_size[0]):
                border = Border(self, self.STARTX + self.PADDING + self.GRIDSIZE * matrix_size[1] * col + 3 * col,
                                ...
                                self.GRIDSIZE * matrix_size[1] + 3, self.GRIDSIZE * matrix_size[0] + 3,
                                3)  # Create matrix borders
    ...
    def __create_number_grid(self, curr_board, orig_board):  # Create number grid method
        ...
        for ridx in range(MATRIX_SIZE[1]):  # Loop through number of rows
            for cidx in range(MATRIX_SIZE[0]):  # Loop through number of cols
                num_input = Button(self, num := to_letter(ridx * MATRIX_SIZE[0] + cidx + 1),
                                   STARTX + NUM_INP_SIZE[1] * cidx, STARTY + NUM_INP_SIZE[0] * ridx, NUM_INP_SIZE[1],
                                   NUM_INP_SIZE[0],
                                   self._application.account.app_config.regular_font, 20,
                                   partial(self.__place_num, num))  # Create input button, connected to place_num method
                num_input.setStyleSheet(
                    f"background: {self._application.account.app_config.colour2}; border: 2px solid black;")  # Set style sheet
        ...
        self.__num_font_size, self.__note_font_size, self.__total_font_size = self.NUM_FONT_SIZES[BOARD_SIZE], \
                                                                              self.NOTE_FONT_SIZES[BOARD_SIZE], \
                                                                              self.TOTAL_FONT_SIZES[BOARD_SIZE]
        ...
        for row, row_lst in enumerate(curr_board):  # Loop through rows
            for col, sq in enumerate(row_lst):  # Loop through squares
                square = Button(window=self,
                                ...
                                # Display note if num is 0
                                x=self.STARTX + self.PADDING + self.GRIDSIZE * col + MATRIX_SIZE[1] * (
                                            col // MATRIX_SIZE[1]),
                ...
                square.setFont(QFont(self._application.account.app_config.regular_font,
                                     self.__num_font_size if sq.num != 0 else self.__note_font_size))  # Set the font
                square.setStyleSheet(
                    f"border: {5 if (row + 1, col + 1) == self.__selected_square else 2}px solid black; background-color:" + (
                        (
                            self._application.account.app_config.colour2 if self.__game.mode == "Normal" else "#C8C8C8")  # Selected square renders as blue in NormalModeBoard, grey in KillerModeBoard
                        if (row + 1, col + 1) == self.__selected_square else
                        ("white" if self.__game.mode == 'Normal' else
                         self._application.account.app_config.killer_colours[self.__colours[(row,
                                                                                             col)]])) +  # Square renders with red/yellow/blue/green in KillerModeBoard, white in NormalModeBoard
                ...
                if self.__game.mode == "Killer" and (row,
                                                     col) in self.__game.groups:  # Create label for total of group if board is a KillerModeBoard and the square is the head of a group
                    total_label = Label(self, str(self.__game.groups[(row, col)][1]), square.x(), square.y(),
                                        square.width() // 3, square.height() // 3,
                                        self._application.account.app_config.regular_font, self.__total_font_size)
    ...
    def __update_number_grid(self, curr_board, orig_board):  # Update number grid after every move
        ...
        for row, row_lst in enumerate(self.__sqrs):  # Loop through rows
            for col, sq in enumerate(row_lst):  # Loop through squares
                sq.setText(str(num) if (num := to_letter(curr_board[row][col].num)) != "0" else self.__game.note_at(row,
                                                                                                                    col))  # Update text
                sq.setFont(QFont(self._application.account.app_config.regular_font, int(mult * (
                    self.__num_font_size if num != "0" else self.__note_font_size))))  # Update font
                sq.setStyleSheet(
                    f"border: {3 if (row + 1, col + 1) == self.__selected_square else 2}px solid black; background-color:" +
                    ((self._application.account.app_config.colour2 if self.__game.mode == "Normal" else "#C8C8C8") if (
                                                                                                                      row + 1,
                                                                                                                      col + 1) == self.__selected_square else (
                        "white" if self.__game.mode == 'Normal' else
                        self._application.account.app_config.killer_colours[self.__colours[(row, col)]])) +
    ...
    def __show_end_screen(self, win):  # method to show game ending screen
        ...
        self.__selected_square = (
        None, None)  # Reset selected square so no squares are rendered in a different colour in the solution
        ...
        if self._application.signed_in and self.__game.timed:  # If signed in to an account and the game is timed
            ...
            self._application.update_rating(
                rating_change := self.__game.rating_change(self._application.account.rating, win))
        ...
        title = Label(self, "You Won!" if win else "Game Over!", 0, 50, 1000, 100,
                      self._application.account.app_config.title_font, 40)
        ...
        if self.__game.timed:  # If the game is timed
            ...
            time_label_top = Label(self, "Time Elapsed: ", 0, 140, 1000, 100,
                                   self._application.account.app_config.regular_font, 20)
            ...
            time = Label(self, self.__game.time_elapsed, 0, 180, 1000, 100,
                         self._application.account.app_config.regular_font, 60)
            ...
            if self._application.signed_in:  # If signed in to an account
                ...
                rating_label_top = Label(self, "New Rating: ", 0, 270, 1000, 100,
                                         self._application.account.app_config.regular_font, 20)
                ...
                # Display rating
                rating = Label(self, rating_str := str(self._application.account.rating), 0, 310, 1000, 100,
                               self._application.account.app_config.regular_font, 60)
                ...
                rating_change = Label(self, f"({f'+{rating_change}' if rating_change >= 0 else rating_change})",
                                      60 * len(rating_str), 350, 1000, 100,
                                      self._application.account.app_config.regular_font, 20)
        ...
        home_screen_button = Button(self, "RETURN TO HOME", 350, 450, 300, 50,
                                    self._application.account.app_config.regular_font, 20, self._return_to_home_screen)
        home_screen_button.setStyleSheet(
            f"background: {self._application.account.app_config.colour2}; border: 2px solid black;")
        home_screen_button.show()

        # Create button to redirect user to solution screen
        solution_button = Button(self, "SEE SOLUTION", 350, 390, 300, 50,
                                 self._application.account.app_config.regular_font, 20, self.__show_solution_screen)
        solution_button.setStyleSheet(
            f"background: {self._application.account.app_config.colour2}; border: 2px solid black;")
    ...
    def __show_solution_screen(self):  # Method to show solution screen
        ...
        # Create button to return back to home screen
        home_screen_button = Button(self, "RETURN TO HOME", 625, 250, 300, 50,
                                    self._application.account.app_config.regular_font, 20, self._return_to_home_screen)
        home_screen_button.setStyleSheet(
            f"background: {self._application.account.app_config.colour2}; border: 2px solid black;")
        home_screen_button.show()
    ...
    def __show_game_paused_error(
            self):  # Method to show game paused error (triggered when user presses buttons that are disabled during game pause)
        self.statusBar().showMessage("ERROR: This action cannot be performed while the game is paused")

    def __select_square(self, row,
                        col):  # Method to select square on the board and update the highlighted selected square
        ...
        if self.__running:  # Check if game is still running
            try:
                ...
                if self.__notes_mode:  # If user is in notes mode
                    ...
                    if self.__game.get_num_at(self.__selected_square[0],
                                              self.__selected_square[1]) == 0:  # Check if number at square is 0
                        self.__game.put_down_number(self.__selected_square[0], self.__selected_square[1],
                                                    num)  # Place number
                    else:
                        self.__game.remove_number(self.__selected_square[0],
                                                  self.__selected_square[1])  # Remove number
        ...
        if self.__running:  # Check if game is running
            try:
                ...
                if not self._application.signed_in:  # No bonus hints if user is not signed in
                    ...
                    bonus_hint_str = f'(+{bonus_hints})' if (
                                                                bonus_hints := self._application.account.bonus_hints) != 0 and self.__game.num_hints_left > 0 else ''
                self.__num_hints_label.setText(
                    f"{self.__game.num_hints_left} {bonus_hint_str}")  # Update number of hints left label
            except GameError as err:
                self.show_error(err)  # Show error if any
            self.__update_curr_grid()  # Update grid
        else:
    ...
    def __toggle_notes_mode(self):  # Method to toggle between notes and normal mode
        if self.__running:  # Check if game is running
            ...
            self.__notes_button.setIcon(QIcon(
                "resources/notes_on.svg" if self.__notes_mode else "resources/notes_off.svg"))  # Switch icon from blue to grey / grey to blue background colour
        else:
            self.__show_game_paused_error()  # Show game paused error if game is not running
    ...
    def _return_to_home_screen(
            self):  # Method to return back to home screen (Overriding existing return to home screen method)
...
class CreateNewAccountScreen(Screen):  # Create new account screen class
    ...
    def __init__(self, application, max_size):  # Constructor
        ...
        super().__init__(application=application, max_size=max_size, title_name="CREATE NEW ACCOUNT",
                         create_button=True)  # Inheritance

        # Create username label and input box
        self.__username = LineEdit(self, 400, 200, 500, 50, self._application.account.app_config.regular_font, 15,
                                   "Type here: ", False)
        self.__username_label = Label(self, "Username: ", 100, 200, 300, 50,
                                      self._application.account.app_config.regular_font, 20)

        # Create password label and input box
        self.__password = LineEdit(self, 400, 300, 500, 50, self._application.account.app_config.regular_font, 15,
                                   "Type here: ", True)
        self.__password_label = Label(self, "Password: ", 100, 300, 300, 50,
                                      self._application.account.app_config.regular_font, 20)

        # Create password (again) label and input box
        self.__password2 = LineEdit(self, 400, 400, 500, 50, self._application.account.app_config.regular_font, 15,
                                    "Type here: ", True)
        self.__password_label2 = Label(self, "Enter password again: ", 100, 400, 300, 50,
                                       self._application.account.app_config.regular_font, 20)

        # Create create account button
        self.__create = Button(self, "Create", 400, 500, 200, 50, self._application.account.app_config.regular_font, 20,
                               self.__create_account)
        self.__create.setStyleSheet(
            f"background: {self._application.account.app_config.colour2}; border: 2px solid black;")
...
class SignInScreen(Screen):  # Sign In Screen class
    ...
    def __init__(self, application, max_size):  # Constructor
        ...
        super().__init__(application=application, max_size=max_size, title_name="SIGN IN",
                         create_button=True)  # Inheritance
        ...
        self.__sign_in = Button(self, "Sign In", 400, 400, 200, 50, self._application.account.app_config.regular_font,
                                20, self.__sign_in)
        self.__sign_in.setStyleSheet(
            f"background: {self._application.account.app_config.colour2}; border: 2px solid black;")
...
class ViewGUIPresetsScreen(Screen):  # View GUI Presets Screen class
    ...
    def __init__(self, application, max_size):  # Constructor
        ...
        super().__init__(application=application, max_size=max_size, title_name="VIEW PRESETS",
                         create_button=True)  # Inheritance

        # Create create preset button
        self.__create_preset = Button(self, "CREATE PRESET", 600, 180, 300, 100,
                                      self._application.account.app_config.regular_font, 22, self.__create_preset)
        self.__create_preset.setStyleSheet(
            f"background: {self._application.account.app_config.killer_colours[3]}; border: 2px solid black;")

        # Create edit preset button
        self.__edit_preset = Button(self, "EDIT PRESET", 600, 310, 300, 100,
                                    self._application.account.app_config.regular_font, 22, self.__edit_preset)
        self.__edit_preset.setStyleSheet(
            f"background: {self._application.account.app_config.colour3}; border: 2px solid black;")

        # Create use preset button
        self.__use_preset = Button(self, "USE PRESET", 600, 440, 300, 100,
                                   self._application.account.app_config.regular_font, 22, self.__use_preset)
        self.__use_preset.setStyleSheet(
            f"background: {self._application.account.app_config.colour3}; border: 2px solid black;")
        ...
        self.__choose_preset = Label(self, "CHOOSE A PRESET", 50, 200, 300, 50,
                                     self._application.account.app_config.regular_font, 20)

        # Create choose preset menu (multiple choice drop down box)
        self.__choose_preset_menu = ComboBox(self, 50, 245, 400, 50, self._application.account.app_config.regular_font,
                                             15,
                                             [f"Preset {preset[0]}" for preset in
                                              database.get_all_presets(self._application.account.username)])
        self.__choose_preset_menu.setStyleSheet(
            f"background: {self._application.account.app_config.colour2}; border: 2px solid black;")
        ...
        self.__preset_preview = TextEdit(self, 50, 325, 400, 235, self._application.account.app_config.colour2, 2,
                                         self._application.account.app_config.regular_font, 14)
    ...
    def __create_preset(self):  # Create preset method
        self.update_preset_signal.emit(["create", database.next_preset_number(
            self._application.account.username)])  # Redirect to edit gui preset screen
    ...
    def __edit_preset(self):  # Edit preset method
        if (text := self.__choose_preset_menu.currentText()):  # If user chose an option in the combobox
            if (
            text := self.__choose_preset_menu.currentText()) != "Preset 1":  # if user did not choose Preset 1 (the default preset)
    ...
    def __use_preset(self):  # Use preset method
        if (text := self.__choose_preset_menu.currentText()):  # If user chose an option in the combobox
            if self._application.account.app_preset_num != (
            num := int(text.split(" ")[1])):  # If preset is not currently being used already
                self._application.use_gui_preset(num)  # Use the preset
                self.__current_preset.setText(
                    f"CURRENT PRESET: Preset {self._application.account.app_preset_num}")  # Update label
            else:
    ...
    def __show_preset_info(self):  # Show preset info in textedit method
        if (text := self.__choose_preset_menu.currentText()):  # If user chose an option in the combobox
            self.__edit_preset.setStyleSheet(
                f"background: {self._application.account.app_config.killer_colours[3]}; border: 2px solid black;")  # change button colour to green
            self.__use_preset.setStyleSheet(
                f"background: {self._application.account.app_config.killer_colours[3]}; border: 2px solid black;")  # change button colour to green
            ...
            labels = ["Background Colour", "Colour 1", "Colour 2", "Colour 3", "Colour 4", "Title Font",
                      "Regular Font", "Killer Colour 1", "Killer Colour 2", "Killer Colour 3", "Killer Colour 4",
                      "Killer Colour 5"]  # initialise labels
...
class EditGUIPresetScreen(Screen):  # Edit GUI Preset Screen Class
    ...
    def __init__(self, application, max_size, mode, preset_num):  # Constructor
        ...
        super().__init__(application=application, max_size=max_size,
                         title_name=f"{'EDIT' if mode == 'edit' else 'CREATE'} PRESET {preset_num}",
                         create_button=True)  # Inheritance
        ...
        for idx, label in enumerate(
                ("Background Colour", "Colour 1", "Colour 2", "Colour 3", "Colour 4", "Title Font")):
            label_obj = Label(self, label, 50, 150 + 60 * idx, 300, 40,
                              self._application.account.app_config.regular_font, 18)
            self._widgets.append(label_obj)

        # Create labels on the right
        for idx, label in enumerate(("Regular Font", "Killer Colour 1", "Killer Colour 2", "Killer Colour 3",
                                     "Killer Colour 4", "Killer Colour 5")):
            label_obj = Label(self, label, 575, 150 + 60 * idx, 300, 40,
                              self._application.account.app_config.regular_font, 18)
            self._widgets.append(label_obj)

        # Create input boxes for user to type in colours (on the left)
        self.__bg_colour = LineEdit(self, 325, 150, 120, 50, self._application.account.app_config.regular_font, 14,
                                    self.__curr_preset[0], False)
        self.__colour1 = LineEdit(self, 325, 210, 120, 50, self._application.account.app_config.regular_font, 14,
                                  self.__curr_preset[1], False)
        self.__colour2 = LineEdit(self, 325, 270, 120, 50, self._application.account.app_config.regular_font, 14,
                                  self.__curr_preset[2], False)
        self.__colour3 = LineEdit(self, 325, 330, 120, 50, self._application.account.app_config.regular_font, 14,
                                  self.__curr_preset[3], False)
        self.__colour4 = LineEdit(self, 325, 390, 120, 50, self._application.account.app_config.regular_font, 14,
                                  self.__curr_preset[4], False)

        # Combo box for user to choose title font from list of system fonts
        self.__title_font = ComboBox(self, 325, 450, 200, 50, self._application.account.app_config.regular_font, 14,
                                     QFontDatabase.families(), add_blank=False)
        ...
        self.__regular_font = ComboBox(self, 775, 150, 200, 50, self._application.account.app_config.regular_font, 14,
                                       QFontDatabase.families(), add_blank=False)
        ...
        self.__killer_colour1 = LineEdit(self, 775, 210, 120, 50, self._application.account.app_config.regular_font, 14,
                                         self.__curr_preset[7], False)
        self.__killer_colour2 = LineEdit(self, 775, 270, 120, 50, self._application.account.app_config.regular_font, 14,
                                         self.__curr_preset[8], False)
        self.__killer_colour3 = LineEdit(self, 775, 330, 120, 50, self._application.account.app_config.regular_font, 14,
                                         self.__curr_preset[9], False)
        self.__killer_colour4 = LineEdit(self, 775, 390, 120, 50, self._application.account.app_config.regular_font, 14,
                                         self.__curr_preset[10], False)
        self.__killer_colour5 = LineEdit(self, 775, 450, 120, 50, self._application.account.app_config.regular_font, 14,
                                         self.__curr_preset[11], False)

        # Create list of options for easy access later
        self.__options = [self.__bg_colour, self.__colour1, self.__colour2, self.__colour3, self.__colour4,
                          self.__killer_colour1,
                          self.__killer_colour2, self.__killer_colour3, self.__killer_colour4, self.__killer_colour5]

        # Create save preset button
        self.__save = Button(self, "Save Changes", 275, 525, 200, 50, self._application.account.app_config.regular_font,
                             18, self.__save)
        self.__save.setStyleSheet(
            f"background: {self._application.account.app_config.colour2}; border: 2px solid black;")

        # Create delete preset button
        self.__delete = Button(self, "Delete Preset", 525, 525, 200, 50,
                               self._application.account.app_config.regular_font, 18, self.__delete)
        self.__delete.setStyleSheet(
            f"background: {self._application.account.app_config.colour2}; border: 2px solid black;")

        # Add to widgets
        self._widgets += [self.__bg_colour, self.__colour1, self.__colour2, self.__colour3, self.__colour4,
                          self.__title_font,
                          self.__regular_font, self.__killer_colour1, self.__killer_colour2, self.__killer_colour3,
                          self.__killer_colour4, self.__killer_colour5,
    ...
    def __font_options_changed(
            self):  # Method to check if font options have been changed from previously saved settings
    ...
    def __save(self):  # save preset method
        ...
        if any([textbox.text() for textbox in
                self.__options]) or self.__font_options_changed():  # check if at least one option is changed
...
class ManageAccountScreen(Screen):  # Manage account screen class
    ...
    def __init__(self, application, max_size):  # Constructor
        ...
        super().__init__(application=application, max_size=max_size, title_name="MANAGE ACCOUNT",
                         create_button=True)  # Inheritance

        # Create labels
        for idx, label in enumerate(("Change Username", "Old Password", "New Password", "New Password Again")):
            label_obj = Label(self, label, 100, 200 + 75 * idx, 300, 50,
                              self._application.account.app_config.regular_font, 18)
            self._widgets.append(label_obj)

        # Create username, old password, new password and new password (again) input boxes for user to type in
        self.__username = LineEdit(self, 400, 200, 500, 50, self._application.account.app_config.regular_font, 15,
                                   f"Current username: {self._application.account.username}", False)
        self.__old_password = LineEdit(self, 400, 275, 500, 50, self._application.account.app_config.regular_font, 15,
                                       "**************", True)
        self.__new_password = LineEdit(self, 400, 350, 500, 50, self._application.account.app_config.regular_font, 15,
                                       "Type here: ", True)
        self.__new_password2 = LineEdit(self, 400, 425, 500, 50, self._application.account.app_config.regular_font, 15,
                                        "Type here: ", True)
        self.__textboxes = [self.__username, self.__old_password, self.__new_password, self.__new_password2]

        # Create save changes button
        self.__save = Button(self, "Save Changes", 275, 500, 200, 50, self._application.account.app_config.regular_font,
                             18, self.__save)
        self.__save.setStyleSheet(
            f"background: {self._application.account.app_config.colour2}; border: 2px solid black;")

        # Create delete account button
        self.__delete = Button(self, "Delete Account", 525, 500, 200, 50,
                               self._application.account.app_config.regular_font, 18, self.__delete)
        self.__delete.setStyleSheet(
            f"background: {self._application.account.app_config.colour4}; border: 2px solid black;")
    ...
    def __save(self):  # Save account changes method
        try:
            ...
            if all([textbox.text() for textbox in self.__textboxes[1:]]):  # If user wishes to change their password
                if not self._application.check_password_match(
                        self.__old_password.text()):  # check if old passwords don't match
    ...
    def __delete(self):  # delete account method
        self.__message_box = QMessageBox(
            self)  # create message box to ask user if they really want to delete their account
        ...
        self.__message_box.setStandardButtons(
            QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No)  # add yes and no buttons
...
class ViewStatsScreen(Screen):  # View stats screen class
    ...
    def __init__(self, application, max_size):  # Constructor
        ...
        super().__init__(application=application, max_size=max_size, title_name="PLAYER STATS",
                         create_button=True)  # Inheritance

        # Create stats text edit widget
        self.__stats = TextEdit(self, 50, 150, 400, 450, self._application.account.app_config.colour2_translucent, 4,
                                self._application.account.app_config.regular_font, 18)
        ...
        for idx, label in enumerate(["Mode", "Board Size", "Difficulty"]):
            label_obj = Label(self, label, 525, 150 + idx * 60, 200, 45,
                              self._application.account.app_config.regular_font, 20)
            self._widgets.append(label_obj)

        # Create menus to choose which stats to view
        self.__mode_menu = ComboBox(self, 700, 150, 200, 45, self._application.account.app_config.regular_font, 20,
                                    ["Normal", "Killer"])
        self.__mode_menu.setStyleSheet(
            f"background: {self._application.account.app_config.colour2}; border: 2px solid black;")
        self.__mode_menu.activated.connect(self.update_gamemode_stats)
        self.__board_size_menu = ComboBox(self, 700, 210, 200, 45, self._application.account.app_config.regular_font,
                                          20, ["4x4", "6x6", "9x9", "12x12", "16x16"])
        self.__board_size_menu.setStyleSheet(
            f"background: {self._application.account.app_config.colour2}; border: 2px solid black;")
        self.__board_size_menu.activated.connect(self.update_gamemode_stats)
        self.__difficulty_menu = ComboBox(self, 700, 270, 200, 45, self._application.account.app_config.regular_font,
                                          20, ["Easy", "Medium", "Hard", "Expert"])
        self.__difficulty_menu.setStyleSheet(
            f"background: {self._application.account.app_config.colour2}; border: 2px solid black;")
        ...
        # create stats text edit widget
        self.__gamemode_stats = TextEdit(self, 525, 350, 400, 250,
                                         self._application.account.app_config.colour2_translucent, 4,
                                         self._application.account.app_config.regular_font, 18)
    ...
    def update_gamemode_stats(self):  # Method to update gamemode stats
        if self.__mode_menu.currentText() and self.__board_size_menu.currentText() and self.__difficulty_menu.currentText():  # Check if user typed something in all three boxes
            ...
            mode, board_size, difficulty = self.__mode_menu.currentText(), int(
                self.__board_size_menu.currentText().split("x")[0]), self.__difficulty_menu.currentText()
...
class GameMilestonesScreen(Screen):  # Game Milestones Screen class
    ...
    def __init__(self, application, max_size):  # Constructor
        ...
        super().__init__(application=application, max_size=max_size, title_name="GAME MILESTONES",
                         create_button=True)  # Inheritance
        ...
        for idx, label in enumerate(milestone_types := ("4x4", "6x6", "9x9", "12x12", "16x16")):
            label = Label(self, label, 50, 175 + 80 * idx, 100, 50, self._application.account.app_config.regular_font,
                          30)
        ...
        for vidx, board_size in enumerate(milestone_types):  # Loop through milestone types
            for hidx, milestone_num in enumerate(range(1, 8)):  # Loop through 7 reward tiers
                ...
                box.setStyleSheet(
                    f"background: {self._application.account.app_config.killer_colours[3] if self.__complete(board_size, milestone_num) else 'rgb(175, 175, 175)'}; border: 3px solid black;")  # Set style sheet
                unclaimed_label = Label(self, "!" if int(milestone_claimed[vidx * 7 + hidx]) else "", x + 48, y + 5, 14,
                                        14,
                                        self._application.account.app_config.regular_font,
                                        14)  # Set label to show if reward is unlocked but unclaimed
                unclaimed_label.setStyleSheet("background: transparent; color: red;")  # Set colour to red
                self.__milestone_buttons[(vidx, hidx)] = (
                board_size, milestone_num, box, unclaimed_label)  # Add to dictionary
                self._widgets.extend([box, unclaimed_label])  # Add to widgets

        self.__milestone_data_box = TextEdit(self, 725, 175, 250, 330,
                                             self._application.account.app_config.colour2_translucent, 3,
                                             self._application.account.app_config.regular_font,
                                             16)  # Create text box to show details about milestone reward
        self.__claim_reward = Button(self, "Claim Reward", 725, 515, 250, 40,
                                     self._application.account.app_config.regular_font, 18,
                                     self.__claim_reward)  # Create claim reward button
        self.__claim_reward.setStyleSheet(
            "background: rgb(175, 175, 175); border: 2px solid black;")  # Set background colour
    ...
    def __update_milestone_grid(self):  # Method to update milestone grid
        ...
        for k, v in self.__milestone_buttons.items():  # Loop through all milestone buttons
            ...
            box.setStyleSheet(
                f"background: {self._application.account.app_config.killer_colours[3] if self.__complete(board_size, milestone_num) else 'rgb(175, 175, 175)'}; border: 3px solid black;")  # Update style sheet
            unclaimed_label.setText("!" if int(milestone_claimed[vidx * 7 + hidx]) else "")  # Set unclaimed label (!)
        self.__view_game_milestone(self.__selected_milestone[0], self.__selected_milestone[
            1])  # View game milestone for the currently selected milestone
    ...
    def __selected_milestone_not_claimed(
            self):  # Method to check if the currently selected milestone reward hasn't been claimed
        ...
        idx = GameMilestones.BOARD_SIZE_IDXS[int(board_size.split("x")[0])] * 7 + int(
            milestone_num) - 1  # get index of reward
    ...
    def __view_game_milestone(self, board_size, milestone_num):  # Method to view game milestone on text edit
        ...
        milestone = database.milestone(self._application.account.username,
                                       f"milestone_{board_size}")  # get milestone number
        status = "Complete" if self.__complete(board_size,
                                               milestone_num) else "Incomplete"  # Set completion status of milestone
        ...
        self.__claim_reward.setStyleSheet(
            f"background: {self._application.account.app_config.killer_colours[3] if self.__complete(board_size, milestone_num) and self.__selected_milestone_not_claimed() else 'rgb(175, 175, 175)'}; border: 3px solid black;")  # Set style sheet
    ...
    def __claim_reward(self):  # Method to claim reward
        if self.__selected_milestone is not None:  # Check if user selected a milestone
            if self.__complete(self.__selected_milestone[0],
                               self.__selected_milestone[1]):  # Check if the milestone has been reached
                if self.__selected_milestone_not_claimed():  # Check if the milestone reward hasn't been claimed yet
                    self._application.claim_reward([int(self.__selected_milestone[0].split("x")[0]),
                                                    int(self.__selected_milestone[1])])  # Claim milestone
...
class HelpScreen(Screen):  # Help Screen class
    ...
    def __init__(self, application, max_size):  # Constructor
        ...
        self.setStyleSheet(
            f"background: {self._application.account.app_config.colour3};")  # Set background colour of screen
...
class LeaderboardScreen(Screen):  # Leaderboard Screen Class
    ...
    def __init__(self, application, max_size):  # Constructor
        ...
        super().__init__(application=application, max_size=max_size, title_name="LEADERBOARD",
                         create_button=True)  # Inheritance

        # Create leaderboard table
        self.__table = TableWidget(self, 50, 125, 600, 450, self._application.account.app_config.regular_font, 18,
                                   self._application.account.app_config.colour2_translucent, 25, 3)
        ...
        self.__type_label = Label(self, "Leaderboard Options", 700, 125, 250, 40,
                                  self._application.account.app_config.regular_font, 18)

        # Create drop down menu where user can choose to view best time or milestone stats
        self.__type = ComboBox(self, 700, 175, 250, 40, self._application.account.app_config.regular_font, 18,
                               ["Best Time", "Milestone"])
        self.__type.setStyleSheet(
            f"background: {self._application.account.app_config.colour2}; border: 2px solid black;")
        ...
        self.__gamemode = ComboBox(self, 700, 275, 250, 40, self._application.account.app_config.regular_font, 18,
                                   gamemode_options)
        self.__gamemode.setStyleSheet(
            f"background: {self._application.account.app_config.colour2}; border: 2px solid black;")
        ...
        self.__milestone = ComboBox(self, 700, 275, 250, 40, self._application.account.app_config.regular_font, 18,
                                    milestone_options)
        self.__milestone.setStyleSheet(
            f"background: {self._application.account.app_config.colour2}; border: 2px solid black;")
    ...
    def __update_table(self, mode):  # Method to update table
        if mode == "gamemode" and (
        txt := self.__gamemode.currentText()):  # If user chose to see gamemode best time stats and user chose an option in the gamemode combobox
            ...
            self.__table.load_data(headings, database.leaderboard_best_time_data(mode, board_size,
                                                                                 difficulty))  # Load data with contents
        elif mode == "milestone" and (
        board_size := self.__milestone.currentText()):  # If user chose to see milestone stats and user chose an option in the milestone combobox
...
class GUI(UI):  # Graphical User Interface (GUI) class
    ...
    def __init__(self):  # Constructor
        ...
        self.__screen_partials = {"home": self.__home_screen,
                                  ...
                                  "view stats": self.__view_stats_screen,
                                  "game milestones": self.__game_milestones_screen,
    ...
    def __home_screen(self):  # Initialise home screen
        ...
        home_screen.create_new_game_signal.connect(
            partial(self.__show_screen, "config game", self.__config_game_screen))
        home_screen.open_existing_game_signal.connect(partial(self.__show_screen, "open game", self.__open_game_screen))
        home_screen.create_new_account_signal.connect(
            partial(self.__show_screen, "create new account", self.__create_new_account_screen))
        ...
        home_screen.manage_account_signal.connect(
            partial(self.__show_screen, "manage account", self.__manage_account_screen))
        home_screen.customise_gui_signal.connect(
            partial(self.__show_screen, "view gui presets", self.__view_gui_presets_screen))
        home_screen.view_stats_signal.connect(partial(self.__show_screen, "view stats", self.__view_stats_screen))
        home_screen.game_milestones_signal.connect(
            partial(self.__show_screen, "game milestones", self.__game_milestones_screen))
