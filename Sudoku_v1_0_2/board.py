...
class Square:  # Square class
    ...
    def note_str(self):  # string representation of note (to render in gui)
        return "\n".join([" " + " ".join([to_letter(j + 1) if self.__note[j] else " " for j in
                                          range(i * self.__MATRIX_SIZE[1], (i + 1) * self.__MATRIX_SIZE[1])]) for i in
                          range(self.__MATRIX_SIZE[0])])

    def pieced_note_str(self, piece):  # string representation of note (to render in terminal)
        return "".join([to_letter(i + 1) if self.__note[i] else " " for i in
                        range((piece - 1) * self.__MATRIX_SIZE[0], piece * self.__MATRIX_SIZE[0])])
...
class Board:  # Board Base Class
    ...
    def __init__(self, board_size):  # Constructor (takes board size : int)
        ...
        self._row_digits = [0 for _ in
                            range(self._board_size)]  # Row digits 1D array of integers to quickly determine legal moves
...
class NormalModeBoard(Board):  # Normal Mode Board Class
    ...
    def is_safe(self, row, col, num):  # Is safe method to check if a number can be placed in a certain square
        return self._not_in_row(row, num) and self._not_in_col(col, num) and self._not_in_3x3_matrix(row, col,
                                                                                                     num)  # safe only if num isn't in row, col, and matrix
...
class KillerModeBoard(Board):  # Killer Mode Board Class
    ...
    def adjacent_non_grouped_cells(self, row,
                                   col):  # gets adjacent cells of a square that aren't grouped (returns list)
        ...
        for squares, total in self._groups.values():  # Iterate through all groups
            if (row, col) in squares:  # Found the group that the square is in
                ...
                if num_blank == 0:  # Group is full
                    return sum([self.get_num_at(sq[0], sq[1]) for sq in
                                squares]) == total  # Check if sum of group is equal to total
