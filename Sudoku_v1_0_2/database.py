...
def next_preset_number(username):  # Get next appearance preset number (local, relative to user)
    if (num := __fetch_data(f"""SELECT MAX(preset_number) FROM AppearancePresets WHERE username='{username}';""")[0][
        0]) is not None:
        return num + 1
    else:
        return 1
...
def create_new_account(username, password):  # Query to create new account when given username and pasword
    ...
    __update_db(
        f"""INSERT INTO Accounts VALUES('{username}', 0, 'New Player', '{encrypt_password(password)}', 1, 0, 0, 0, 0, 0, '{'0' * 35}', 0);""")
...
def password_at(
        username):  # Query to get password of a given username, used to check against password inputted when signing in
    __setup()
...
def get_all_presets(
        username):  # Query to get all appearance presets of a given account, used in ViewGUIPresetsScreen to display all stored presets to the user
    __setup()
...
def get_preset(username,
               preset_num):  # Query to get appearance preset of a given preset number for a given account, used to display settings when user browses stored presets
    __setup()
    return __fetch_data(f"""SELECT background_colour, colour1, colour2, colour3, colour4, title_font, 
                        regular_font, killer_colour1, killer_colour2, killer_colour3, killer_colour4, killer_colour5 
...
def update_appearance_preset(username, preset_num,
                             options):  # Query to edit existing appearance preset, used when user edits a preset and saves it
    __setup()
    __update_db(
        f"""UPDATE AppearancePresets SET background_colour='{options[0]}', colour1='{options[1]}', colour2='{options[2]}',
...
def create_new_appearance_preset(username,
                                 options):  # Query to create new appearance preset, used when user saves a newly created preset
    __setup()
...
def get_current_appearance_preset_num(
        username):  # Query to get preset number (local) of current appearance preset that is in use
    __setup()
    return __fetch_data(f"""SELECT current_appearance_preset_number FROM Accounts WHERE username='{username}';""")[0][0]


def set_current_appearance_preset(username,
                                  preset_num):  # Query to set current appearance preset given preset number and username
    __setup()
...
def change_username(orig_username,
                    new_username):  # Query to change username given original username and new username (ManageAccount screen)
    try:
        __setup()
...
def change_password(username,
                    new_password):  # Query to change password given username and new password (ManageAccount screen)
    __setup()
...
def times_played(username, mode, board_size,
                 difficulty):  # Query to get the number of times a user played a certain gamemode
    __setup()
...
def num_completions(username, mode, board_size,
                    difficulty):  # Query to get the number of times a user completed a board of a certain gamemode
    __setup()
    return __fetch_data(f"""SELECT COUNT(*) FROM Games WHERE username='{username}' 
...
def best_hardcore_time(username, mode, board_size,
                       difficulty):  # Query to get the user's best hardcore (no hints) time in a certain gamemode
    __setup()
    secs = __fetch_data(f"""SELECT MIN(time_to_complete) FROM Games WHERE username='{username}' 
                            AND mode='{mode}' AND board_size={board_size} AND difficulty='{difficulty}'
...
def milestone(username,
              board_size):  # Query to get the milestone number of an account given the username and board size
    __setup()
    return __fetch_data(f"""SELECT {board_size} FROM Accounts WHERE username='{username}';""")[0][0]


def milestone_claimed(
        username):  # Query to get the claimed string of a given account that encodes whether the user has claimed a reward for each milestone
    __setup()
...
def set_milestone_claimed(username,
                          claimed):  # Query to set the claimed string of a given account after user claims a reward or unlocks a new milestone
    __setup()
...
def set_bonus_hints(username,
                    num_hints):  # Query to update the numnber of bonus hints an account currently has after claiming rewards
    __setup()
...
def leaderboard_milestone_data(
        board_size):  # Query to get the username, rating, title and milestone number of all registered accounts (used in leaderboard)
