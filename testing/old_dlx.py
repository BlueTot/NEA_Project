...
def solve_sudoku(board): # Main solver function (takes NormalModeBoard object)
    ...
    for solution_set in dlx_solve(cols, rows, []): # Iterate through all solutions of the board using DLX Solver
...
def dlx_solve(cols, rows, solution_set):
    if not cols: # no remaining columns (constraints)
        ...
        for chosen_row in list(cols[chosen_col]): # Iterate through all rows that intersect with the chosen column
            ...
            for sol in dlx_solve(cols, rows, solution_set): # recursively solve the reduced matrix
                yield sol
