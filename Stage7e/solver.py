...
class BoardSolver: # Board Solver Class
    ...
    @staticmethod
    def __solvable(board, row=0, col=0): # Simple backtracking solver that returns True and solves the board if it is solvable, otherwise it returns False
    ...
    @staticmethod
    def solver(board, generating_mode=False): # Solver function interface, calls DLX SOLVER if board is NormalModeBoard, otherwise calls SIMPLE BACKTRACKING SOLVER
    ...
    @staticmethod
    def __num_solutions(board, row=0, col=0, num_sols=0): # Method to count the number of solutions using backtracking (for generating killer mode boards)
        ...
        if board.get_num_at(row, col) > 0: # if number is not 0
            return BoardSolver.__num_solutions(board, row, col+1, num_sols) # recursively solve starting from next square
        
        for num in range(1, board.board_size + 1): # try all numbers
            if board.is_safe(row, col, num): # if number can be placed (is safe)
                board.set_num_at(row, col, num) # set number at (row, col)
                if (num_sols := BoardSolver.__num_solutions(board, row, col+1, num_sols)) > 1: # check if board is still unique
                    break 
            board.set_num_at(row, col, 0) # undo (set back to 0)
              
        return num_sols
    
    def is_unique(board): # Is Unique Function interface, calls DLX SOLVER if board is NormalModeBoard, otherwise calls SIMPLE BACKTRACKING SOLVER
